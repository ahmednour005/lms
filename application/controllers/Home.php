...
class Home extends CI_Controller {
    ...
    public function index() {
        ...
        if($this->input->post('contactSubmit')){
            ...
            $this->form_validation->set_rules('formName', 'Name', 'required');
            $this->form_validation->set_rules('formemail', 'Email', 'required|valid_email');
    ...
    public function contact() {
        ...
        $page_data['page_title'] = site_phrase('contact');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function payment_gateway_mobile($course_id = "", $user_id = ""){
        if($course_id > 0 && $user_id > 0):
           ...
           public function courseform() {
            $page_data['page_name'] = 'courseform';
            $page_data['page_title'] = site_phrase('courseform');
            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
        }

       // PAYOUT REPORT
       public function depart1() {
        $page_data['page_name'] = 'depart1';
        $page_data['page_title'] = site_phrase('depart1');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }

    public function depart2() {
        $page_data['page_name'] = 'depart2';
        $page_data['page_title'] = site_phrase('depart2');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }

    public function existCources() {
        $page_data['page_name'] = 'existCources';
        $page_data['page_title'] = site_phrase('existCources');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
